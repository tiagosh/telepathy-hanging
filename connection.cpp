    ...
    mDisconnectReason(Tp::ConnectionStatusReasonRequested)
{
    ...
    setConnectCallback(Tp::memFun(this,&HangingConnection::connect));
    setInspectHandlesCallback(Tp::memFun(this,&HangingConnection::inspectHandles));
    setRequestHandlesCallback(Tp::memFun(this,&HangingConnection::requestHandles));
    setCreateChannelCallback(Tp::memFun(this,&HangingConnection::createChannel));
    ...
    simplePresenceIface->setSetPresenceCallback(Tp::memFun(this,&HangingConnection::setPresence));
    ...
    contactListIface->setGetContactListAttributesCallback(Tp::memFun(this, &HangingConnection::getContactListAttributes));
...
void HangingConnection::onAuthFailed(AuthenticationStatus status, QString reason)
{
    QVariantMap details;
...
void HangingConnection::onLoginNeeded()
{
    mHangishClient->sendCredentials(mAccount, mPassword);
}

void HangingConnection::on2FactorAuthRequired()
{
    Tp::DBusError error;
...
void HangingConnection::onAvatarDownloaded()
{
...
Tp::AvatarTokenMap HangingConnection::getKnownAvatarTokens(const Tp::UIntList &contacts, Tp::DBusError *error)
{
    const QStringList identifiers = inspectHandles(Tp::HandleTypeContact, contacts, error);
...
Tp::ContactAttributesMap HangingConnection::getContactListAttributes(const QStringList &interfaces, bool hold, Tp::DBusError *error)
{
    Q_UNUSED(hold);
...
void HangingConnection::onInitFinished()
{
    if (!saslIface.isNull()) {
...
HangishClient *HangingConnection::hangishClient()
{
    return mHangishClient;
}

HangingTextChannel* HangingConnection::textChannelForConversationId(const QString &conversationId)
{
...
Tp::ContactAttributesMap HangingConnection::getContactAttributes(const Tp::UIntList &handles, const QStringList &ifaces, Tp::DBusError *error)
{
...
QStringList HangingConnection::inspectHandles(uint handleType, const Tp::UIntList& handles, Tp::DBusError *error)
{
    ...
    qDebug() << "HangingConnection::inspectHandles " << identifiers;
    return identifiers;
}
...
QString HangingConnection::conversationIdForChatId(const QString &chatId)
{
...
QMap<QString, ClientConversationState> HangingConnection::getConversations()
{
...
Tp::BaseChannelPtr HangingConnection::createRoomListChannel()
{
    ...
    roomListChannel->setListRoomsCallback(Tp::memFun(this, &HangingConnection::roomListStartListing));
    roomListChannel->setStopListingCallback(Tp::memFun(this, &HangingConnection::roomListStopListing));
    baseChannel->plugInterface(Tp::AbstractChannelInterfacePtr::dynamicCast(roomListChannel));

    return baseChannel;
}

void HangingConnection::roomListStartListing(Tp::DBusError *error)
{
    Q_UNUSED(error)
...
void HangingConnection::roomListStopListing(Tp::DBusError *error)
{
    Q_UNUSED(error)
...
Tp::BaseChannelPtr HangingConnection::createChannel(const QVariantMap &request, Tp::DBusError *error)
{
...
void HangingConnection::onClientSyncAllNewEventsResponse(ClientSyncAllNewEventsResponse &csaner)
{
...
void HangingConnection::onClientQueryPresenceResponse(quint64,ClientQueryPresenceResponse &cqprp)
{
...
HangingTextChannel* HangingConnection::ensureTextChannel(const QString &conversationId, const QString &fromId)
{
    QVariantMap request;
    ClientConversationState conversation = mConversations[conversationId];
...
void HangingConnection::onClientStateUpdate(ClientStateUpdate &update)
{
...
void HangingConnection::onTextChannelClosed()
{
...
uint HangingConnection::ensureRoomHandle(const QString &id)
{
    ...
    qDebug() << "HangingConnection::ensureRoomHandle" << id;
    return newRoomHandle(id);
}

uint HangingConnection::ensureContactHandle(const QString &id)
{
    ...
    qDebug() << "HangingConnection::ensureContactHandle" << id;
    return newContactHandle(id);
}

uint HangingConnection::newContactHandle(const QString &identifier)
{
    mContactHandles[++mHandleCount] = identifier;
    return mHandleCount;
}

uint HangingConnection::newRoomHandle(const QString &identifier)
{
